...
def create_collection():
    collection = URIRef("https://w3id.org/hto/WorkCollection/LadiesEdinburghDebatingSociety")
    graph.add((collection, RDF.type, hto.WorkCollection))
    graph.add((collection, hto.name, Literal("Ladies’ Edinburgh Debating Society Collection", datatype=XSD.string)))
    return collection
...
def run_task(inputs):
    print("---- Start the gazetteer  dataframe to rdf task ----")
    gazetteer_dataframes = inputs["dataframes"]
    ...
    for dataframe in gazetteer_dataframes:
        filename = dataframe["filename"]
        file_path = "../../source_dataframes/ladies/" + filename
        print(f"Parsing dataframe {filename} to graph....")
        agent = dataframe["agent"]
        agent_uri = create_organization(agent, graph)
        eb_dataset = create_dataset("ladies", agent_uri, agent, graph)
